...
void ReadData1(void){
	...
	while(!feof(fp)){
		...
		printf("ReadData1 stack[0] =%d stack[1]=%d sem.value = %d\n",stack[0],stack[1],size);
		sem_post(&sem);
		//sleep(1);
	}
	fclose(fp);
...
void ReadData2(void){
	...
	while(!feof(fp)){
		fscanf(fp,"%d %d",&stack[0],&stack[1]);
		sem_getvalue(&sem,&size);
		printf("ReadData2 stack[0] =%d stack[1]=%d sem.value = %d\n",stack[0],stack[1],size);
		sem_post(&sem);
		//sleep(1);
	}
	fclose(fp);
...
void HandleData2(void){
	...
	while(1){
		sem_getvalue(&sem,&size);
		sem_wait(&sem);
		printf("Multiply:%d*%d=%d sem.value = %d\n",stack[0],stack[1],stack[0]*stack[1],size);
		//sleep(1);
	}
}
